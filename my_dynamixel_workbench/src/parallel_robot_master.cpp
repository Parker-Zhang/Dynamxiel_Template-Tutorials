/**********************************************************************
parallel_robot_master.cpp
Date:2020.12.16
Organized by Zhenwei Zhang | email：1742356236@qq.com
Descripotion:This is a controller for self-design parallel_robot_master
SHOW_ME mode give you a way to record robot trajectory guided by human.
init_goal pos is absolute pos.
...
#include<ros/ros.h>
#include<parallel_robot_master.h>

using namespace std;

#define SHOW_ME_MODE 0
#define ABSOLUTE_INIT_POS 1
...
bool MyDynamixelController::getPresentPosition(std::vector<std::string> dxl_name)
{
  ...
  else
  {
    ...
    {
      #if ABSOLUTE_INIT_POS == 0
      wp.position = dxl_wb_->convertValue2Radian(id_array[index], get_position[index]-init_pos[index_array[index]]+2048); 
      #endif

      #if ABSOLUTE_INIT_POS == 1
      wp.position = dxl_wb_->convertValue2Radian(id_array[index], get_position[index]+2048); 
      cout<<"first point wp pos:"<< wp.position<<endl;
      #endif
...
//记录上电时舵机的位置信息，保存到 init_pos 和 goal_pos 中
...
void MyDynamixelController::writeCallback(const ros::TimerEvent&){
  ...
  if (is_moving_==true)
  {
    ...
    #if ABSOLUTE_INIT_POS == 0
      goal_pos[index_array[index]] = dynamixel_position[index]-2048 + init_pos[index_array[index]]; //2048 为0°的位置，这是为了记录相对角度
    #endif
    #if ABSOLUTE_INIT_POS == 1
...
{
    ros::init(argc,argv,"parallel_robot_master");
